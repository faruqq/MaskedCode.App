...
internal sealed class EglNumericLiteralMasker
{
        ...
        out IReadOnlyDictionary<string, string> mappings)
    {
        ...
        var usedMaskedValues =
            new HashSet<string>(
                StringComparer.Ordinal);

        var originalNumericLiterals =
            CollectOriginalNumericLiterals(sourceCode);
        ...
        while (index < sourceCode.Length)
        {
            ...
            if (!numericMappings.TryGetValue(
                    literal,
        ...
        out NumericLiteralKind kind)
    {
                ...
                sourceCode[index]))
        {
            ...
            var exponentDigitStart =
                exponentCursor;

            while (exponentCursor < sourceCode.Length &&
                   char.IsDigit(
                       sourceCode[exponentCursor]))
            {
                exponentCursor++;
            }
        ...
        ISet<string> originalNumericLiterals)
    {
             ...
             attempt++)
        {
            ...
            if (string.Equals(
                ...
                originalNumericLiterals.Contains(
                    candidate) ||
                usedMaskedValues.Contains(
                    candidate))
            {
                continue;
            }
    ...
    private static string CreateMaskedLiteral(
        string literal,
        NumericLiteralKind kind)
    {
        var mantissaEnd =
            kind == NumericLiteralKind.Scientific
    ...
    private static bool IsBuiltInTypeArgument(
        string sourceCode,
        int literalStartIndex)
    {
        var openingParenthesisIndex =
        ...
        return EglKeywordCatalog.IsBuiltInType(
            identifier);
    }

    private static int FindEnclosingOpeningParenthesis(
        string sourceCode,
        int startIndex)
    {
        var depth = 0;
        ...
        int literalLength)
    {
        ...
        {
            var typeName =
                ReadTypeNameBeforeClosingParenthesis(
                    sourceCode,
                    index);
        ...
        if (EglKeywordCatalog.IsBuiltInType(
                typeNameValue))
        {
            return true;
        }
        ...
        return !EglKeywordCatalog.IsKeyword(
                   declarationName) &&
               !EglKeywordCatalog.IsBuiltInType(
                   declarationName);
    }
    ...
    private static bool IsMetadataNumericValue(
        string sourceCode,
        int literalStartIndex)
    {
        var index =
        ...
        return EglKeywordCatalog.IsMetadataProperty(
            propertyName);
    }
    ...
    private static void SkipIdentifier(
        string sourceCode,
        ref int index)
    {
        index++;
    ...
    private static void SkipStringLiteral(
        string sourceCode,
        ref int index)
    {
        index++;
    ...
    private static void SkipWhitespaceForward(
        string sourceCode,
        ref int index)
    {
        while (index < sourceCode.Length &&
